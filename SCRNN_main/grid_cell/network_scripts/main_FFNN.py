...
learning_rate = 0.001
dropout = 0.2
nn_layers = 2   #number of layers of NN
nn_width = [256, 256, 256]    #list of width of layers of neural network
...
#Load data
if end_time=='end':
	file = open('../analyses/' + str(win_size*1000) + 'ms/interval'  + '.p', 'rb')
	...
	file = open('../analyses/' + str(win_size*1000) + 'ms/interval2' + 'mod2.p', 'rb')
	spike_data_dict = pickle.load(file)
	file.close()
	spike_count_matrix2 = spike_data_dict['count_matrix']  #load spike count matrix of second module

	file = open('../analyses/' + str(win_size*1000) + 'ms/interval2' + 'mod3.p', 'rb')
	spike_data_dict = pickle.load(file)
	file.close()
	...
	file = open('../analyses/' + str(win_size*1000) + 'ms/interval2' + 'mod1.p', 'rb')
	spike_data_dict = pickle.load(file)
	file.close()
