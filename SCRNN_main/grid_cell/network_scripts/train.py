...
def train(model, device, data_loader, optimizer, criterion, scheduler, epochs):
	...
	np.savetxt(lrs_file   + ".txt", np.array(lrs), fmt = '%1.18f', delimiter=',')
	np.savetxt(loss_file   + ".txt", np.array(losses), fmt = '%1.18f', delimiter=',')
...
def trainFFNN(model, device, data_loader, optimizer, criterion, scheduler, epochs):
	...
	now = str(datetime.now())
	dt = now.replace(":", "-").replace(" ", "_")

	# Ensure the output folder exists
	output_dir = os.path.join(sys.path[0], "extracted")
	os.makedirs(output_dir, exist_ok=True)

	lrs_file = os.path.join(output_dir, "lrs"   + ".txt")
	loss_file = os.path.join(output_dir, "loss"   + ".txt")

	np.savetxt(lrs_file, np.array(lrs), fmt='%1.18f', delimiter=',')
	np.savetxt(loss_file, np.array(losses), fmt='%1.18f', delimiter=',')
